...
class GlueTaskRunner:
    def __init__(self, model, optimizer, scheduler, tokenizer, label_list, device, rparams):
        self.model = model
    ...
    def run_train_val_test(self, train_examples, val_examples, test_examples, task_name):
        epoch_result_dict_val = col.OrderedDict()
        epoch_result_dict_test = col.OrderedDict()
        for i in trange(int(self.rparams.num_train_epochs), desc="Epoch"):
            train_dataloader = self.get_train_dataloader(train_examples, verbose=True)
            self.run_train_epoch(train_dataloader)
            epoch_result_val = self.run_val(val_examples, task_name, verbose=True)
            epoch_result_test = self.run_val(test_examples, task_name, verbose=True)
    ...
    def run_train_step(self, step, batch, train_epoch_state):
        ...
        loss = self.model(input_ids=batch.input_ids, token_type_ids=batch.segment_ids, attention_mask=batch.input_mask, labels=batch.label_ids)[0]
        ...
        if (step + 1) % self.rparams.gradient_accumulation_steps == 0:
            ...
            torch.nn.utils.clip_grad_norm_(self.glue_lm_model.parameters(), 1.0)
            self.optimizer.step()
            self.scheduler.step()
    ...
    def run_val(self, val_examples, task_name, verbose=True):
        ...
        for step, batch in enumerate(tqdm(val_dataloader, desc="Evaluating (Val)")):
            ...
            with torch.no_grad():
                tmp_eval_loss = self.model(input_ids=batch.input_ids, token_type_ids=batch.segment_ids, attention_mask=batch.input_mask, labels=batch.label_ids)[0]
                logits = self.model(input_ids=batch.input_ids, token_type_ids=batch.segment_ids, attention_mask=batch.input_mask)[0]
